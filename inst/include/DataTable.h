...
class DataTable {
  ...
  */
  DataTable(const string &file, const string &headerKey="",  const string &delim = "\t "){

    if( ! filesystem::exists( file ) ){
      throw logic_error("File does not exist: " + file);
    }

    boost::char_separator<char> separators( delim.c_str() );
  ...
  void setColNames(const vector<string> &names) {
    if( names.size() != colNames.size()){
      ...
      // Rcpp::Rcout << "names: " << names.size() << std::endl;
      // for(auto x: names) Rcpp::Rcout << x << std::endl;
      // Rcpp::Rcout << "colNames: " << colNames.size() << std::endl;
      // for(auto x: colNames) Rcpp::Rcout << x << std::endl;
